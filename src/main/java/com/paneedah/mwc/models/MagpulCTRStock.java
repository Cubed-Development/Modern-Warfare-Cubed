package com.paneedah.mwc.models;

import net.minecraft.client.model.ModelBase;
import net.minecraft.client.model.ModelBox;
import net.minecraft.client.model.ModelRenderer;
import net.minecraft.entity.Entity;

// Made with Blockbench 3.8.4
// Exported for Minecraft version 1.7 - 1.12
// Paste this class into your mod and generate all required imports


public class MagpulCTRStock extends ModelBase {
    private final ModelRenderer gun365;
    private final ModelRenderer gun366;
    private final ModelRenderer gun367;
    private final ModelRenderer gun368;
    private final ModelRenderer gun369;
    private final ModelRenderer gun370;
    private final ModelRenderer gun371;
    private final ModelRenderer gun372;
    private final ModelRenderer gun373;
    private final ModelRenderer gun374;
    private final ModelRenderer gun375;
    private final ModelRenderer gun376;
    private final ModelRenderer gun378;
    private final ModelRenderer gun380;
    private final ModelRenderer gun381;
    private final ModelRenderer gun382;
    private final ModelRenderer gun383;
    private final ModelRenderer gun384;
    private final ModelRenderer gun385;
    private final ModelRenderer gun386;
    private final ModelRenderer gun387;
    private final ModelRenderer gun389;
    private final ModelRenderer gun390;
    private final ModelRenderer gun391;
    private final ModelRenderer gun392;
    private final ModelRenderer gun393;
    private final ModelRenderer gun394;
    private final ModelRenderer gun395;
    private final ModelRenderer gun396;
    private final ModelRenderer gun397;
    private final ModelRenderer gun398;
    private final ModelRenderer gun399;
    private final ModelRenderer gun400;
    private final ModelRenderer gun402;
    private final ModelRenderer gun403;
    private final ModelRenderer gun404;
    private final ModelRenderer gun405;
    private final ModelRenderer gun406;
    private final ModelRenderer gun407;
    private final ModelRenderer gun408;
    private final ModelRenderer gun409;
    private final ModelRenderer gun410;
    private final ModelRenderer gun411;
    private final ModelRenderer gun412;
    private final ModelRenderer gun413;
    private final ModelRenderer gun414;
    private final ModelRenderer gun415;
    private final ModelRenderer gun416;
    private final ModelRenderer gun417;
    private final ModelRenderer gun418;
    private final ModelRenderer gun419;
    private final ModelRenderer gun420;

    public MagpulCTRStock() {
        textureWidth = 90;
        textureHeight = 90;

        gun365 = new ModelRenderer(this);
        gun365.setRotationPoint(-2, -11.5F, 3.5F);
        gun365.cubeList.add(new ModelBox(gun365, 10, 25, 0, 0, 0, 1, 3, 4, 0, false));

        gun366 = new ModelRenderer(this);
        gun366.setRotationPoint(-3, -10.5F, 3.5F);
        gun366.cubeList.add(new ModelBox(gun366, 26, 32, 0, 0, 0, 3, 1, 4, 0, false));

        gun367 = new ModelRenderer(this);
        gun367.setRotationPoint(-2, -11.7F, 8);
        gun367.cubeList.add(new ModelBox(gun367, 0, 21, 0, 0, 0, 1, 3, 20, 0, false));

        gun368 = new ModelRenderer(this);
        gun368.setRotationPoint(-2, -9.3F, 7.5F);


        gun369 = new ModelRenderer(this);
        gun369.setRotationPoint(-3.2F, -10.5F, 7.5F);


        gun370 = new ModelRenderer(this);
        gun370.setRotationPoint(-1.8F, -10.5F, 7.5F);


        gun371 = new ModelRenderer(this);
        gun371.setRotationPoint(-1, -11.7F, 7.5F);
        setRotationAngle(gun371, 0, 0, 0.7854F);
        gun371.cubeList.add(new ModelBox(gun371, 0, 0, 0, 0, 0, 2, 1, 20, 0, false));

        gun372 = new ModelRenderer(this);
        gun372.setRotationPoint(-2, -11.7F, 7.5F);
        setRotationAngle(gun372, 0, 0, 0.7854F);
        gun372.cubeList.add(new ModelBox(gun372, 24, 1, 0, 0, 0, 1, 2, 20, 0, false));

        gun373 = new ModelRenderer(this);
        gun373.setRotationPoint(-3.2F, -10.5F, 7.5F);
        setRotationAngle(gun373, 0, 0, -0.7854F);


        gun374 = new ModelRenderer(this);
        gun374.setRotationPoint(-3.2F, -9.5F, 7.5F);
        setRotationAngle(gun374, 0, 0, -0.7854F);
        gun374.cubeList.add(new ModelBox(gun374, 44, 44, 0, 0, 0, 1, 1, 20, 0, false));

        gun375 = new ModelRenderer(this);
        gun375.setRotationPoint(0.2F, -10.5F, 7.5F);
        setRotationAngle(gun375, 0, 0, 2.3562F);


        gun376 = new ModelRenderer(this);
        gun376.setRotationPoint(0.2F, -9.5F, 7.5F);
        setRotationAngle(gun376, 0, 0, 2.3562F);
        gun376.cubeList.add(new ModelBox(gun376, 44, 23, 0, 0, 0, 1, 1, 20, 0, false));

        gun378 = new ModelRenderer(this);
        gun378.setRotationPoint(-2.5F, -9.3F, 7.5F);
        gun378.cubeList.add(new ModelBox(gun378, 0, 21, 0, 0, 0, 2, 3, 5, 0, false));

        gun380 = new ModelRenderer(this);
        gun380.setRotationPoint(-2.5F, 0.9F, 24.4F);
        gun380.cubeList.add(new ModelBox(gun380, 30, 29, 0, 0, 0, 2, 1, 2, 0, false));

        gun381 = new ModelRenderer(this);
        gun381.setRotationPoint(-3.3F, -10.9F, 27.5F);
        setRotationAngle(gun381, -0.1115F, 0, 0);
        gun381.cubeList.add(new ModelBox(gun381, 24, 0, 0, 0, 0.001F, 3, 12, 1, 0, false));

        gun382 = new ModelRenderer(this);
        gun382.setRotationPoint(-0.7F, -10.9F, 27.5F);
        setRotationAngle(gun382, -0.1115F, 0, 0);
        gun382.cubeList.add(new ModelBox(gun382, 16, 7, 0, 0, 0, 1, 12, 1, 0, false));

        gun383 = new ModelRenderer(this);
        gun383.setRotationPoint(-2.5F, -11.8F, 27.6F);
        setRotationAngle(gun383, -0.1115F, 0, 0);
        gun383.cubeList.add(new ModelBox(gun383, 36, 29, 0, 0, 0, 2, 1, 1, 0, false));

        gun384 = new ModelRenderer(this);
        gun384.setRotationPoint(-3, -11.4F, 27.55F);
        setRotationAngle(gun384, -0.1115F, 0, 0);
        gun384.cubeList.add(new ModelBox(gun384, 33, 16, 0, 0, 0, 3, 1, 1, 0, false));

        gun385 = new ModelRenderer(this);
        gun385.setRotationPoint(-2, -11.7F, 7.5F);
        gun385.cubeList.add(new ModelBox(gun385, 0, 7, 0, -0.001F, 0, 1, 3, 1, 0, false));

        gun386 = new ModelRenderer(this);
        gun386.setRotationPoint(-2.5F, -9.8F, 12.5F);
        gun386.cubeList.add(new ModelBox(gun386, 46, 0, 0, 0, 0, 2, 2, 15, 0, false));

        gun387 = new ModelRenderer(this);
        gun387.setRotationPoint(-2.5F, -7.9F, 12.5F);
        gun387.cubeList.add(new ModelBox(gun387, 33, 10, 0.001F, 0, 0, 2, 3, 3, 0, false));

        gun389 = new ModelRenderer(this);
        gun389.setRotationPoint(-2.5F, -7.8F, 23.5F);
        gun389.cubeList.add(new ModelBox(gun389, 0, 34, 0.002F, 0, 0, 2, 1, 4, 0, false));

        gun390 = new ModelRenderer(this);
        gun390.setRotationPoint(-2.5F, -7.3F, 25.1F);
        setRotationAngle(gun390, -0.1115F, 0, 0);
        gun390.cubeList.add(new ModelBox(gun390, 0, 0, 0.001F, 0, 0, 2, 5, 1, 0, false));

        gun391 = new ModelRenderer(this);
        gun391.setRotationPoint(-2, -7.1F, 25.5F);
        setRotationAngle(gun391, -0.1115F, 0, 0);
        gun391.cubeList.add(new ModelBox(gun391, 44, 45, 0, 0, 0, 1, 6, 2, 0, false));

        gun392 = new ModelRenderer(this);
        gun392.setRotationPoint(-2.5F, -6.8F, 23.5F);
        setRotationAngle(gun392, 1.1154F, 0, 0);
        gun392.cubeList.add(new ModelBox(gun392, 36, 32, 0, 0, 0, 2, 2, 1, 0, false));

        gun393 = new ModelRenderer(this);
        gun393.setRotationPoint(-2.5F, -1.1F, 21.6F);
        gun393.cubeList.add(new ModelBox(gun393, 24, 13, 0.001F, 0, 0, 2, 1, 5, 0, false));

        gun394 = new ModelRenderer(this);
        gun394.setRotationPoint(-2.5F, 1, 26.2F);
        setRotationAngle(gun394, -0.1115F, 0, 0);
        gun394.cubeList.add(new ModelBox(gun394, 14, 32, 0, 0, 0, 2, 1, 1, 0, false));

        gun395 = new ModelRenderer(this);
        gun395.setRotationPoint(-3, 0.5F, 26.25F);
        setRotationAngle(gun395, -0.1115F, 0, 0);
        gun395.cubeList.add(new ModelBox(gun395, 12, 4, 0, 0, 0, 3, 1, 1, 0, false));

        gun396 = new ModelRenderer(this);
        gun396.setRotationPoint(-2.5F, -6.8F, 23.5F);
        setRotationAngle(gun396, 2.082F, 0, 0);
        gun396.cubeList.add(new ModelBox(gun396, 9, 21, -0.001F, 0, 0, 2, 1, 2, 0, false));

        gun397 = new ModelRenderer(this);
        gun397.setRotationPoint(-3, -8.5F, 13);
        setRotationAngle(gun397, -0.1115F, 0, 0);
        gun397.cubeList.add(new ModelBox(gun397, 0, 0, 0, 0, 0, 3, 1, 6, 0, false));

        gun398 = new ModelRenderer(this);
        gun398.setRotationPoint(-3, -7.9F, 18.95F);
        setRotationAngle(gun398, -0.2603F, 0, 0);
        gun398.cubeList.add(new ModelBox(gun398, 8, 34, -0.001F, 0, 0, 3, 1, 3, 0, false));

        gun399 = new ModelRenderer(this);
        gun399.setRotationPoint(-3, -7, 17.5F);
        gun399.cubeList.add(new ModelBox(gun399, 0, 29, 0.001F, 0, 0, 3, 1, 4, 0, false));

        gun400 = new ModelRenderer(this);
        gun400.setRotationPoint(-3, -6, 17.5F);
        setRotationAngle(gun400, 2.8256F, 0, 0);
        gun400.cubeList.add(new ModelBox(gun400, 0, 14, -0.001F, 0, 0, 3, 1, 5, 0, false));

        gun402 = new ModelRenderer(this);
        gun402.setRotationPoint(-2.5F, -2.8F, 24.6F);
        setRotationAngle(gun402, -0.7854F, 0, 0);
        gun402.cubeList.add(new ModelBox(gun402, 28, 23, 0, 0, 0, 2, 1, 1, 0, false));

        gun403 = new ModelRenderer(this);
        gun403.setRotationPoint(-2, -0.9F, 21.5F);
        gun403.cubeList.add(new ModelBox(gun403, 30, 23, 0, 0, 0, 1, 1, 5, 0, false));

        gun404 = new ModelRenderer(this);
        gun404.setRotationPoint(-2, 0.7F, 24);
        gun404.cubeList.add(new ModelBox(gun404, 12, 0, 0, 0, 0, 1, 1, 3, 0, false));

        gun405 = new ModelRenderer(this);
        gun405.setRotationPoint(-2, 0.1F, 22.5F);
        setRotationAngle(gun405, 0.7808F, 0, 0);
        gun405.cubeList.add(new ModelBox(gun405, 11, 7, -0.001F, 0, 0, 1, 2, 1, 0, false));

        gun406 = new ModelRenderer(this);
        gun406.setRotationPoint(-2, -0.1F, 26.1F);
        setRotationAngle(gun406, -0.1487F, 0, 0);
        gun406.cubeList.add(new ModelBox(gun406, 11, 10, -0.001F, 0, 0, 1, 1, 1, 0, false));

        gun407 = new ModelRenderer(this);
        gun407.setRotationPoint(-1, -11.5F, 3.5F);
        setRotationAngle(gun407, 0, 0, 0.7854F);
        gun407.cubeList.add(new ModelBox(gun407, 32, 45, 0, -0.001F, 0, 1, 1, 4, 0, false));

        gun408 = new ModelRenderer(this);
        gun408.setRotationPoint(-2, -11.5F, 3.5F);
        setRotationAngle(gun408, 0, 0, 0.7854F);
        gun408.cubeList.add(new ModelBox(gun408, 22, 45, -0.001F, 0, 0, 1, 1, 4, 0, false));

        gun409 = new ModelRenderer(this);
        gun409.setRotationPoint(-3, -10.5F, 3.5F);
        setRotationAngle(gun409, 0, 0, -0.7854F);
        gun409.cubeList.add(new ModelBox(gun409, 44, 38, 0, 0, 0, 1, 1, 4, 0, false));

        gun410 = new ModelRenderer(this);
        gun410.setRotationPoint(-3, -9.5F, 3.5F);
        setRotationAngle(gun410, 0, 0, -0.7854F);
        gun410.cubeList.add(new ModelBox(gun410, 44, 33, -0.001F, 0, 0, 1, 1, 4, 0, false));

        gun411 = new ModelRenderer(this);
        gun411.setRotationPoint(0, -10.5F, 3.5F);
        setRotationAngle(gun411, 0, 0, 2.3562F);
        gun411.cubeList.add(new ModelBox(gun411, 44, 28, 0, 0, 0, 1, 1, 4, 0, false));

        gun412 = new ModelRenderer(this);
        gun412.setRotationPoint(0, -9.5F, 3.5F);
        setRotationAngle(gun412, 0, 0, 2.3562F);
        gun412.cubeList.add(new ModelBox(gun412, 44, 23, 0, -0.001F, 0, 1, 1, 4, 0, false));

        gun413 = new ModelRenderer(this);
        gun413.setRotationPoint(-2, -8.5F, 3.5F);
        setRotationAngle(gun413, 0, 0, -2.3562F);
        gun413.cubeList.add(new ModelBox(gun413, 10, 44, 0, 0, 0, 1, 1, 4, 0, false));

        gun414 = new ModelRenderer(this);
        gun414.setRotationPoint(-1, -8.5F, 3.5F);
        setRotationAngle(gun414, 0, 0, -2.3562F);
        gun414.cubeList.add(new ModelBox(gun414, 0, 44, 0, 0, 0, 1, 1, 4, 0, false));

        gun415 = new ModelRenderer(this);
        gun415.setRotationPoint(-2.5F, -6.3F, 10.1F);
        setRotationAngle(gun415, 1.0647F, 0, 0);
        gun415.cubeList.add(new ModelBox(gun415, 22, 23, 0, 2, 0, 2, 11, 2, 0, false));

        gun416 = new ModelRenderer(this);
        gun416.setRotationPoint(-2.5F, -2.5F, 20.6F);
        gun416.cubeList.add(new ModelBox(gun416, 32, 5, 0.001F, 0, 0, 2, 1, 4, 0, false));

        gun417 = new ModelRenderer(this);
        gun417.setRotationPoint(-0.6F, -10.3F, 7.5F);
        gun417.cubeList.add(new ModelBox(gun417, 0, 44, 0, 0, 0, 1, 1, 20, 0, false));

        gun418 = new ModelRenderer(this);
        gun418.setRotationPoint(-3.4F, -10.3F, 7.5F);
        gun418.cubeList.add(new ModelBox(gun418, 22, 24, 0, 0, 0, 1, 1, 20, 0, false));

        gun419 = new ModelRenderer(this);
        gun419.setRotationPoint(-3, -8, 7.8F);
        gun419.cubeList.add(new ModelBox(gun419, 32, 0, 0, 0, 0, 3, 2, 3, 0, false));

        gun420 = new ModelRenderer(this);
        gun420.setRotationPoint(-3, -8, 10.8F);
        setRotationAngle(gun420, -0.5205F, 0, 0);
        gun420.cubeList.add(new ModelBox(gun420, 0, 7, -0.001F, 0, 0, 3, 2, 5, 0, false));
    }

    @Override
    public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5) {
        gun365.render(f5);
        gun366.render(f5);
        gun367.render(f5);
        gun368.render(f5);
        gun369.render(f5);
        gun370.render(f5);
        gun371.render(f5);
        gun372.render(f5);
        gun373.render(f5);
        gun374.render(f5);
        gun375.render(f5);
        gun376.render(f5);
        gun378.render(f5);
        gun380.render(f5);
        gun381.render(f5);
        gun382.render(f5);
        gun383.render(f5);
        gun384.render(f5);
        gun385.render(f5);
        gun386.render(f5);
        gun387.render(f5);
        gun389.render(f5);
        gun390.render(f5);
        gun391.render(f5);
        gun392.render(f5);
        gun393.render(f5);
        gun394.render(f5);
        gun395.render(f5);
        gun396.render(f5);
        gun397.render(f5);
        gun398.render(f5);
        gun399.render(f5);
        gun400.render(f5);
        gun402.render(f5);
        gun403.render(f5);
        gun404.render(f5);
        gun405.render(f5);
        gun406.render(f5);
        gun407.render(f5);
        gun408.render(f5);
        gun409.render(f5);
        gun410.render(f5);
        gun411.render(f5);
        gun412.render(f5);
        gun413.render(f5);
        gun414.render(f5);
        gun415.render(f5);
        gun416.render(f5);
        gun417.render(f5);
        gun418.render(f5);
        gun419.render(f5);
        gun420.render(f5);
    }

    public void setRotationAngle(ModelRenderer modelRenderer, float x, float y, float z) {
        modelRenderer.rotateAngleX = x;
        modelRenderer.rotateAngleY = y;
        modelRenderer.rotateAngleZ = z;
    }
}
