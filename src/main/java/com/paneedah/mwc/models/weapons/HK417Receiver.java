package com.paneedah.mwc.models.weapons;

import com.paneedah.weaponlib.ModelWithAttachments;
import net.minecraft.client.model.ModelBox;
import net.minecraft.client.model.ModelRenderer;
import net.minecraft.entity.Entity;

// Made with Blockbench 4.9.2
// Exported for Minecraft version 1.7 - 1.12
// Paste this class into your mod and generate all required imports


public class HK417Receiver extends ModelWithAttachments {
	private final ModelRenderer receiver;
	private final ModelRenderer gun259_r1;
	private final ModelRenderer gun263_r1;
	private final ModelRenderer gun142_r1;
	private final ModelRenderer gun140_r1;
	private final ModelRenderer gun105_r1;
	private final ModelRenderer cube_r1;
	private final ModelRenderer cube_r2;
	private final ModelRenderer cube_r3;
	private final ModelRenderer cube_r4;
	private final ModelRenderer cube_r5;
	private final ModelRenderer cube_r6;
	private final ModelRenderer cube_r7;
	private final ModelRenderer gun264;
	private final ModelRenderer gun170;
	private final ModelRenderer gun249;
	private final ModelRenderer gun;
	private final ModelRenderer gun118_r1_r1;
	private final ModelRenderer gun60_r1;
	private final ModelRenderer gun67_r1;
	private final ModelRenderer gun132_r1;
	private final ModelRenderer gun262_r1_r1;
	private final ModelRenderer gun152_r1_r1;
	private final ModelRenderer gun117_r2_r1;
	private final ModelRenderer gun115_r1_r1;
	private final ModelRenderer gun116_r2_r1;
	private final ModelRenderer gun117_r1_r1;
	private final ModelRenderer gun116_r1_r1;
	private final ModelRenderer gun119_r1_r1;
	private final ModelRenderer gun82_r1_r1;
	private final ModelRenderer gun66_r1_r1;
	private final ModelRenderer gun63_r1_r1;
	private final ModelRenderer gun58_r1_r1;
	private final ModelRenderer gun53_r1_r1;
	private final ModelRenderer gun50_r1_r1;
	private final ModelRenderer gun52_r1_r1;
	private final ModelRenderer gun47_r1_r1;
	private final ModelRenderer gun48_r1_r1;
	private final ModelRenderer gun28_r1_r1;
	private final ModelRenderer gun29_r2_r1;
	private final ModelRenderer gun29_r1_r1;
	private final ModelRenderer gun30_r1_r1;
	private final ModelRenderer gun21_r1_r1;
	private final ModelRenderer gun22_r1_r1;

	public HK417Receiver() {
		textureWidth = 220;
		textureHeight = 220;

		receiver = new ModelRenderer(this);
		receiver.setRotationPoint(2.0F, -13.0F, -22.7F);
		receiver.cubeList.add(new ModelBox(receiver, 136, 110, -2.2F, -0.6F, -1.8F, 1, 1, 26, 0.0F, false));
		receiver.cubeList.add(new ModelBox(receiver, 62, 36, -2.7F, -2.15F, 1.95F, 1, 2, 1, 0.0F, false));
		receiver.cubeList.add(new ModelBox(receiver, 45, 30, -4.3F, -1.88F, 21.95F, 1, 1, 2, 0.0F, false));
		receiver.cubeList.add(new ModelBox(receiver, 45, 22, -3.7F, -1.88F, 21.95F, 1, 1, 2, 0.0F, false));
		receiver.cubeList.add(new ModelBox(receiver, 85, 97, -5.8F, -0.6F, -1.8F, 1, 1, 3, 0.0F, false));
		receiver.cubeList.add(new ModelBox(receiver, 75, 20, -5.8F, -0.6F, 0.7F, 1, 1, 1, 0.0F, false));
		receiver.cubeList.add(new ModelBox(receiver, 90, 45, -5.8F, -0.6F, 12.2F, 1, 1, 12, 0.0F, false));
		receiver.cubeList.add(new ModelBox(receiver, 90, 0, -2.199F, 0.4F, -1.8F, 1, 1, 13, 0.0F, false));
		receiver.cubeList.add(new ModelBox(receiver, 70, 89, -5.799F, 0.4F, -1.8F, 1, 1, 13, 0.0F, false));
		receiver.cubeList.add(new ModelBox(receiver, 154, 0, -4.5F, -3.1F, -1.8F, 2, 1, 23, 0.0F, false));
		receiver.cubeList.add(new ModelBox(receiver, 143, 137, -4.6F, -2.8F, -1.801F, 2, 2, 24, 0.0F, false));
		receiver.cubeList.add(new ModelBox(receiver, 54, 47, -4.6F, -2.8F, 22.199F, 2, 1, 1, 0.0F, false));
		receiver.cubeList.add(new ModelBox(receiver, 75, 18, -3.4F, -2.801F, 22.2F, 1, 1, 1, 0.0F, false));
		receiver.cubeList.add(new ModelBox(receiver, 0, 152, -3.4F, -2.8F, -1.801F, 1, 2, 24, 0.0F, false));
		receiver.cubeList.add(new ModelBox(receiver, 63, 45, -6.3F, -0.2F, 1.7F, 1, 1, 10, 0.0F, false));
		receiver.cubeList.add(new ModelBox(receiver, 100, 142, -4.8F, -0.7F, 0.2F, 3, 2, 24, 0.0F, false));
		receiver.cubeList.add(new ModelBox(receiver, 50, 153, -5.25F, 0.3F, 0.7F, 1, 1, 24, 0.0F, false));

		gun259_r1 = new ModelRenderer(this);
		gun259_r1.setRotationPoint(-4.8F, -2.0F, 10.7F);
		receiver.addChild(gun259_r1);
		setRotationAngle(gun259_r1, 0.0F, 0.0F, 0.632F);
		gun259_r1.cubeList.add(new ModelBox(gun259_r1, 64, 11, 0.0375F, 0.2204F, 0.0F, 1, 2, 1, 0.0F, false));
		gun259_r1.cubeList.add(new ModelBox(gun259_r1, 73, 56, 0.0375F, 0.2204F, -9.0F, 1, 2, 1, 0.0F, false));
		gun259_r1.cubeList.add(new ModelBox(gun259_r1, 63, 18, 0.0F, 0.0F, -9.0F, 1, 1, 10, 0.0F, false));

		gun263_r1 = new ModelRenderer(this);
		gun263_r1.setRotationPoint(-6.1F, 0.3F, 1.2F);
		receiver.addChild(gun263_r1);
		setRotationAngle(gun263_r1, 0.0F, 0.0F, -0.9295F);
		gun263_r1.cubeList.add(new ModelBox(gun263_r1, 19, 44, 0.0F, 0.0F, 0.0F, 1, 1, 11, 0.0F, false));

		gun142_r1 = new ModelRenderer(this);
		gun142_r1.setRotationPoint(-6.7F, -0.6F, 18.1F);
		receiver.addChild(gun142_r1);
		setRotationAngle(gun142_r1, 0.0F, -0.2603F, 0.0F);
		gun142_r1.cubeList.add(new ModelBox(gun142_r1, 54, 26, 0.0F, 0.0F, 0.0F, 1, 1, 2, 0.0F, false));

		gun140_r1 = new ModelRenderer(this);
		gun140_r1.setRotationPoint(-5.1F, -0.4F, 11.6F);
		receiver.addChild(gun140_r1);
		setRotationAngle(gun140_r1, 0.0F, -0.2603F, 0.0F);
		gun140_r1.cubeList.add(new ModelBox(gun140_r1, 46, 108, -0.01F, 0.0F, 0.0F, 2, 1, 7, 0.0F, false));
		gun140_r1.cubeList.add(new ModelBox(gun140_r1, 11, 108, -0.01F, -0.4F, 0.0F, 2, 1, 7, 0.0F, false));

		gun105_r1 = new ModelRenderer(this);
		gun105_r1.setRotationPoint(-2.5F, -3.1F, 21.2F);
		receiver.addChild(gun105_r1);
		setRotationAngle(gun105_r1, -0.3142F, 0.0F, 0.0F);
		gun105_r1.cubeList.add(new ModelBox(gun105_r1, 54, 45, -2.0F, 0.0F, 0.0F, 2, 1, 1, 0.0F, false));

		cube_r1 = new ModelRenderer(this);
		cube_r1.setRotationPoint(-5.8F, -0.6F, 0.45F);
		receiver.addChild(cube_r1);
		setRotationAngle(cube_r1, 0.0F, 0.0F, 0.8203F);
		cube_r1.cubeList.add(new ModelBox(cube_r1, 63, 56, 0.0F, -2.0F, 0.25F, 1, 2, 1, 0.0F, false));
		cube_r1.cubeList.add(new ModelBox(cube_r1, 40, 92, 0.0F, -2.0F, -2.25F, 1, 2, 3, 0.0F, false));

		cube_r2 = new ModelRenderer(this);
		cube_r2.setRotationPoint(-5.8F, -0.6F, 13.45F);
		receiver.addChild(cube_r2);
		setRotationAngle(cube_r2, 0.0F, 0.0F, 0.8727F);
		cube_r2.cubeList.add(new ModelBox(cube_r2, 90, 14, 0.0F, -2.0F, -1.75F, 1, 2, 12, 0.0F, false));

		cube_r3 = new ModelRenderer(this);
		cube_r3.setRotationPoint(-1.2F, -0.6F, -0.05F);
		receiver.addChild(cube_r3);
		setRotationAngle(cube_r3, 0.0F, 0.0F, -0.8727F);
		cube_r3.cubeList.add(new ModelBox(cube_r3, 33, 121, -1.0F, -2.0F, -1.75F, 1, 2, 26, 0.0F, false));

		cube_r4 = new ModelRenderer(this);
		cube_r4.setRotationPoint(-5.8F, 0.4F, 11.2F);
		receiver.addChild(cube_r4);
		setRotationAngle(cube_r4, 0.0F, 0.0F, 0.9076F);
		cube_r4.cubeList.add(new ModelBox(cube_r4, 0, 88, 0.0F, -1.0F, 0.0F, 1, 1, 13, 0.0F, false));

		cube_r5 = new ModelRenderer(this);
		cube_r5.setRotationPoint(-1.2F, 0.4F, 11.2F);
		receiver.addChild(cube_r5);
		setRotationAngle(cube_r5, 0.0F, 0.0F, -0.9076F);
		cube_r5.cubeList.add(new ModelBox(cube_r5, 35, 89, -1.0F, -1.0F, 0.0F, 1, 1, 13, 0.0F, false));

		cube_r6 = new ModelRenderer(this);
		cube_r6.setRotationPoint(-1.7F, -0.15F, 1.95F);
		receiver.addChild(cube_r6);
		setRotationAngle(cube_r6, 0.0F, 0.8727F, 0.0F);
		cube_r6.cubeList.add(new ModelBox(cube_r6, 0, 62, -1.0F, -2.0F, -1.0F, 1, 2, 1, 0.0F, false));

		cube_r7 = new ModelRenderer(this);
		cube_r7.setRotationPoint(-1.7F, -0.15F, 2.95F);
		receiver.addChild(cube_r7);
		setRotationAngle(cube_r7, 0.0F, -0.8727F, 0.0F);
		cube_r7.cubeList.add(new ModelBox(cube_r7, 6, 62, -1.0F, -2.0F, 0.0F, 1, 2, 1, 0.0F, false));

		gun264 = new ModelRenderer(this);
		gun264.setRotationPoint(-6.2F, -0.4F, 2.7F);
		receiver.addChild(gun264);
		setRotationAngle(gun264, 0.0F, 0.0F, 0.2974F);
		gun264.cubeList.add(new ModelBox(gun264, 100, 64, -0.0154F, 0.6322F, 0.0F, 1, 3, 8, 0.0F, false));
		gun264.cubeList.add(new ModelBox(gun264, 36, 72, -0.2528F, 1.438F, 3.5F, 1, 2, 1, 0.0F, false));
		gun264.cubeList.add(new ModelBox(gun264, 70, 96, -0.1121F, 0.9868F, 0.25F, 1, 2, 3, 0.0F, false));
		gun264.cubeList.add(new ModelBox(gun264, 35, 95, -0.1121F, 0.9868F, 4.75F, 1, 2, 3, 0.0F, false));

		gun170 = new ModelRenderer(this);
		gun170.setRotationPoint(-5.9F, -2.2F, 12.7F);
		receiver.addChild(gun170);
		setRotationAngle(gun170, 0.0F, 0.0F, -0.192F);
		gun170.cubeList.add(new ModelBox(gun170, 90, 28, -1.3573F, 0.2945F, -1.0F, 3, 2, 1, 0.0F, false));

		gun249 = new ModelRenderer(this);
		gun249.setRotationPoint(-1.3F, 2.0F, 0.0F);
		gun170.addChild(gun249);
		setRotationAngle(gun249, 0.0F, 1.1154F, 0.0F);
		gun249.cubeList.add(new ModelBox(gun249, 78, 11, -0.0252F, -1.7055F, -0.0514F, 1, 2, 4, 0.0F, false));
		gun249.cubeList.add(new ModelBox(gun249, 54, 18, 0.9748F, -1.7055F, 1.9486F, 1, 2, 2, 0.0F, false));

		gun = new ModelRenderer(this);
		gun.setRotationPoint(0.0F, 24.0F, 0.0F);
		gun.cubeList.add(new ModelBox(gun, 54, 22, -0.5F, -33.7F, -5.5F, 1, 1, 2, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 74, 29, -0.15F, -36.2F, -12.75F, 1, 2, 1, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 75, 71, -0.3F, -35.3F, -13.0F, 1, 1, 1, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 75, 73, -0.3F, -35.3F, -12.5F, 1, 1, 1, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 45, 63, -2.5F, -39.0F, 1.001F, 2, 4, 3, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 90, 49, -3.9F, -37.6F, 1.002F, 2, 1, 3, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 90, 45, -1.1F, -37.6F, 1.002F, 2, 1, 3, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 39, 76, -3.7F, -33.6F, -13.0F, 1, 1, 1, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 76, 64, -3.8F, -32.2F, -12.4F, 1, 1, 1, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 54, 29, -3.8F, -32.2F, -13.7F, 1, 1, 2, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 45, 36, -3.8F, -34.2F, -14.7F, 1, 3, 1, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 64, 0, -3.8F, -35.0F, -21.4F, 1, 1, 10, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 61, 36, -3.0F, -34.05F, -1.0F, 3, 1, 5, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 0, 80, -3.0F, -31.7F, -4.0F, 3, 1, 4, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 45, 36, -3.3F, -35.7F, -1.0F, 3, 2, 5, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 75, 64, -0.7F, -35.7F, -1.0F, 1, 2, 5, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 19, 72, -3.5F, -35.4F, -24.5F, 4, 1, 2, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 76, 0, -3.5F, -35.7F, -13.2F, 4, 4, 2, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 45, 63, -3.5F, -35.7F, -23.2F, 4, 8, 10, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 19, 52, -3.0F, -35.7F, -24.501F, 3, 1, 2, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 75, 71, -0.349F, -32.85F, -17.25F, 1, 1, 5, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 26, 36, -0.85F, -33.45F, -17.5F, 2, 1, 6, -0.4F, false));
		gun.cubeList.add(new ModelBox(gun, 75, 25, -0.849F, -33.45F, -13.2F, 2, 1, 2, -0.4F, false));
		gun.cubeList.add(new ModelBox(gun, 45, 10, -0.85F, -32.25F, -17.5F, 2, 1, 6, -0.4F, false));
		gun.cubeList.add(new ModelBox(gun, 34, 68, -0.849F, -32.25F, -13.2F, 2, 1, 2, -0.4F, false));
		gun.cubeList.add(new ModelBox(gun, 45, 49, -0.85F, -33.35F, -17.7F, 2, 2, 1, -0.4F, false));
		gun.cubeList.add(new ModelBox(gun, 93, 97, -0.85F, -33.35F, -13.0F, 2, 2, 2, -0.4F, false));
		gun.cubeList.add(new ModelBox(gun, 103, 98, -0.15F, -35.05F, -22.8F, 1, 1, 8, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 0, 36, -3.5F, -27.7F, -19.2F, 4, 1, 6, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 0, 62, -3.3F, -35.7F, -14.0F, 3, 5, 13, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 60, 68, -0.7F, -35.7F, -14.0F, 1, 5, 13, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 93, 89, -3.0F, -31.7F, -13.8F, 3, 5, 1, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 45, 70, -3.0F, -26.7F, -13.5F, 3, 1, 2, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 32, 8, -3.001F, -26.7F, -7.6F, 3, 1, 2, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 45, 14, -2.501F, -25.95F, -10.0F, 2, 1, 1, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 0, 40, -2.502F, -25.951F, -10.2F, 2, 1, 1, 0.0F, false));
		gun.cubeList.add(new ModelBox(gun, 0, 95, -3.0F, -30.7F, -6.5F, 3, 5, 1, 0.0F, false));

		gun118_r1_r1 = new ModelRenderer(this);
		gun118_r1_r1.setRotationPoint(19.9293F, -66.9451F, 0.0F);
		gun.addChild(gun118_r1_r1);
		setRotationAngle(gun118_r1_r1, 0.0F, 0.0F, -2.3562F);
		gun118_r1_r1.cubeList.add(new ModelBox(gun118_r1_r1, 75, 93, -4.9F, -37.6F, 1.0F, 1, 2, 3, 0.0F, false));

		gun60_r1 = new ModelRenderer(this);
		gun60_r1.setRotationPoint(0.0F, 5.2554F, -12.7938F);
		gun.addChild(gun60_r1);
		setRotationAngle(gun60_r1, -0.3316F, 0.0F, 0.0F);
		gun60_r1.cubeList.add(new ModelBox(gun60_r1, 39, 78, -0.4F, -34.7303F, -23.7043F, 1, 1, 1, 0.0F, false));
		gun60_r1.cubeList.add(new ModelBox(gun60_r1, 36, 77, -3.6F, -34.7303F, -23.7043F, 1, 1, 1, 0.0F, false));

		gun67_r1 = new ModelRenderer(this);
		gun67_r1.setRotationPoint(0.0F, -57.932F, -21.4691F);
		gun.addChild(gun67_r1);
		setRotationAngle(gun67_r1, -2.3423F, 0.0F, 0.0F);
		gun67_r1.cubeList.add(new ModelBox(gun67_r1, 0, 54, -3.29F, -33.6454F, 0.2192F, 3, 2, 5, 0.0F, false));
		gun67_r1.cubeList.add(new ModelBox(gun67_r1, 64, 11, -3.001F, -34.3715F, 0.5877F, 3, 2, 4, 0.0F, false));

		gun132_r1 = new ModelRenderer(this);
		gun132_r1.setRotationPoint(34.4061F, -25.4138F, 0.0F);
		gun.addChild(gun132_r1);
		setRotationAngle(gun132_r1, 0.0F, 0.0F, -1.2641F);
		gun132_r1.cubeList.add(new ModelBox(gun132_r1, 55, 12, -1.0731F, -35.8027F, -13.05F, 2, 1, 1, -0.2F, false));
		gun132_r1.cubeList.add(new ModelBox(gun132_r1, 55, 10, -1.0731F, -35.8027F, -12.45F, 2, 1, 1, -0.2F, false));

		gun262_r1_r1 = new ModelRenderer(this);
		gun262_r1_r1.setRotationPoint(31.509F, -29.3702F, 0.0F);
		gun.addChild(gun262_r1_r1);
		setRotationAngle(gun262_r1_r1, 0.0F, 0.0F, -1.2641F);
		gun262_r1_r1.cubeList.add(new ModelBox(gun262_r1_r1, 72, 37, -4.3F, -36.2F, -21.0F, 1, 2, 1, 0.0F, false));

		gun152_r1_r1 = new ModelRenderer(this);
		gun152_r1_r1.setRotationPoint(0.0F, -14.1789F, 21.6308F);
		gun.addChild(gun152_r1_r1);
		setRotationAngle(gun152_r1_r1, 0.7854F, 0.0F, 0.0F);
		gun152_r1_r1.cubeList.add(new ModelBox(gun152_r1_r1, 75, 47, -0.4F, -33.2F, -6.3F, 1, 1, 1, 0.0F, false));

		gun117_r2_r1 = new ModelRenderer(this);
		gun117_r2_r1.setRotationPoint(-25.0507F, -64.8238F, 0.0F);
		gun.addChild(gun117_r2_r1);
		setRotationAngle(gun117_r2_r1, 0.0F, 0.0F, 2.3562F);
		gun117_r2_r1.cubeList.add(new ModelBox(gun117_r2_r1, 40, 97, 0.9F, -37.6F, 1.0F, 1, 2, 3, 0.0F, false));

		gun115_r1_r1 = new ModelRenderer(this);
		gun115_r1_r1.setRotationPoint(-9.4421F, -1.4801F, 0.0F);
		gun.addChild(gun115_r1_r1);
		setRotationAngle(gun115_r1_r1, 0.0F, 0.0F, 0.2618F);
		gun115_r1_r1.cubeList.add(new ModelBox(gun115_r1_r1, 14, 80, -1.1F, -36.6F, 0.999F, 2, 3, 3, 0.0F, false));

		gun116_r2_r1 = new ModelRenderer(this);
		gun116_r2_r1.setRotationPoint(-7.4807F, -8.8004F, -21.321F);
		gun.addChild(gun116_r2_r1);
		setRotationAngle(gun116_r2_r1, -0.6283F, 0.0F, 0.2618F);
		gun116_r2_r1.cubeList.add(new ModelBox(gun116_r2_r1, 23, 96, -1.099F, -36.6F, 0.999F, 2, 3, 2, 0.0F, false));

		gun117_r1_r1 = new ModelRenderer(this);
		gun117_r1_r1.setRotationPoint(7.3784F, -9.5768F, -21.321F);
		gun.addChild(gun117_r1_r1);
		setRotationAngle(gun117_r1_r1, -0.6283F, 0.0F, -0.2618F);
		gun117_r1_r1.cubeList.add(new ModelBox(gun117_r1_r1, 15, 96, -3.899F, -36.6F, 0.999F, 2, 3, 2, 0.0F, false));

		gun116_r1_r1 = new ModelRenderer(this);
		gun116_r1_r1.setRotationPoint(9.3399F, -2.2565F, 0.0F);
		gun.addChild(gun116_r1_r1);
		setRotationAngle(gun116_r1_r1, 0.0F, 0.0F, -0.2618F);
		gun116_r1_r1.cubeList.add(new ModelBox(gun116_r1_r1, 64, 0, -3.9F, -36.6F, 0.999F, 2, 3, 3, 0.0F, false));

		gun119_r1_r1 = new ModelRenderer(this);
		gun119_r1_r1.setRotationPoint(-7.745F, -1.2973F, 0.0F);
		gun.addChild(gun119_r1_r1);
		setRotationAngle(gun119_r1_r1, 0.0F, 0.0F, 0.2094F);
		gun119_r1_r1.cubeList.add(new ModelBox(gun119_r1_r1, 54, 49, 0.3F, -34.5F, 3.102F, 2, 1, 1, -0.3F, false));
		gun119_r1_r1.cubeList.add(new ModelBox(gun119_r1_r1, 54, 53, 0.3F, -37.5F, 3.102F, 2, 1, 1, -0.3F, false));
		gun119_r1_r1.cubeList.add(new ModelBox(gun119_r1_r1, 38, 13, 1.301F, -37.501F, 3.101F, 1, 4, 1, -0.3F, false));

		gun82_r1_r1 = new ModelRenderer(this);
		gun82_r1_r1.setRotationPoint(8.948F, 0.0F, -6.4526F);
		gun.addChild(gun82_r1_r1);
		setRotationAngle(gun82_r1_r1, 0.0F, 0.7436F, 0.0F);
		gun82_r1_r1.cubeList.add(new ModelBox(gun82_r1_r1, 45, 0, -3.8F, -34.0F, -14.7F, 1, 3, 1, 0.0F, false));

		gun66_r1_r1 = new ModelRenderer(this);
		gun66_r1_r1.setRotationPoint(0.0F, -58.4865F, -21.1033F);
		gun.addChild(gun66_r1_r1);
		setRotationAngle(gun66_r1_r1, -2.3423F, 0.0F, 0.0F);
		gun66_r1_r1.cubeList.add(new ModelBox(gun66_r1_r1, 85, 89, -0.71F, -33.6362F, 0.3041F, 1, 2, 6, 0.0F, false));

		gun63_r1_r1 = new ModelRenderer(this);
		gun63_r1_r1.setRotationPoint(0.0F, 1.0932F, -12.6292F);
		gun.addChild(gun63_r1_r1);
		setRotationAngle(gun63_r1_r1, -0.409F, 0.0F, 0.0F);
		gun63_r1_r1.cubeList.add(new ModelBox(gun63_r1_r1, 0, 44, -2.0F, -31.9F, -8.95F, 1, 4, 1, 0.0F, false));

		gun58_r1_r1 = new ModelRenderer(this);
		gun58_r1_r1.setRotationPoint(0.0F, -32.4771F, 17.5348F);
		gun.addChild(gun58_r1_r1);
		setRotationAngle(gun58_r1_r1, 0.9082F, 0.0F, 0.0F);
		gun58_r1_r1.cubeList.add(new ModelBox(gun58_r1_r1, 63, 25, -3.5F, -34.323F, -24.3423F, 4, 2, 1, 0.0F, false));

		gun53_r1_r1 = new ModelRenderer(this);
		gun53_r1_r1.setRotationPoint(0.0F, -57.5692F, -32.6735F);
		gun.addChild(gun53_r1_r1);
		setRotationAngle(gun53_r1_r1, -2.7884F, 0.0F, 0.0F);
		gun53_r1_r1.cubeList.add(new ModelBox(gun53_r1_r1, 75, 98, -0.501F, -31.7F, -11.2F, 1, 1, 3, 0.0F, false));

		gun50_r1_r1 = new ModelRenderer(this);
		gun50_r1_r1.setRotationPoint(0.0F, -46.0263F, 7.1916F);
		gun.addChild(gun50_r1_r1);
		setRotationAngle(gun50_r1_r1, 1.4192F, 0.0F, 0.0F);
		gun50_r1_r1.cubeList.add(new ModelBox(gun50_r1_r1, 19, 0, -3.8F, -27.3194F, -23.1019F, 4, 10, 1, 0.0F, false));
		gun50_r1_r1.cubeList.add(new ModelBox(gun50_r1_r1, 63, 53, -3.799F, -27.7F, -23.1019F, 4, 1, 1, 0.0F, false));
		gun50_r1_r1.cubeList.add(new ModelBox(gun50_r1_r1, 36, 75, -0.199F, -27.7F, -23.1019F, 1, 1, 1, 0.0F, false));
		gun50_r1_r1.cubeList.add(new ModelBox(gun50_r1_r1, 8, 88, -0.2F, -27.3194F, -23.1019F, 1, 10, 1, 0.0F, false));

		gun52_r1_r1 = new ModelRenderer(this);
		gun52_r1_r1.setRotationPoint(0.0F, 2.1018F, -15.1188F);
		gun.addChild(gun52_r1_r1);
		setRotationAngle(gun52_r1_r1, -0.4363F, 0.0F, 0.0F);
		gun52_r1_r1.cubeList.add(new ModelBox(gun52_r1_r1, 75, 45, 0.05F, -33.05F, -12.3F, 1, 1, 1, -0.2F, false));

		gun47_r1_r1 = new ModelRenderer(this);
		gun47_r1_r1.setRotationPoint(-24.5352F, -10.867F, 0.0F);
		gun.addChild(gun47_r1_r1);
		setRotationAngle(gun47_r1_r1, 0.0F, 0.0F, 0.7854F);
		gun47_r1_r1.cubeList.add(new ModelBox(gun47_r1_r1, 103, 89, -0.15F, -35.05F, -22.8F, 1, 1, 8, 0.0F, false));

		gun48_r1_r1 = new ModelRenderer(this);
		gun48_r1_r1.setRotationPoint(-23.8281F, -10.5741F, 0.0F);
		gun.addChild(gun48_r1_r1);
		setRotationAngle(gun48_r1_r1, 0.0F, 0.0F, 0.7854F);
		gun48_r1_r1.cubeList.add(new ModelBox(gun48_r1_r1, 103, 33, -0.15F, -34.05F, -22.8F, 1, 1, 8, 0.0F, false));

		gun28_r1_r1 = new ModelRenderer(this);
		gun28_r1_r1.setRotationPoint(0.0F, -37.4482F, -38.5114F);
		gun.addChild(gun28_r1_r1);
		setRotationAngle(gun28_r1_r1, -2.3562F, 0.0F, 0.0F);
		gun28_r1_r1.cubeList.add(new ModelBox(gun28_r1_r1, 90, 39, -3.0F, -26.7F, -11.5F, 3, 2, 1, 0.0F, false));

		gun29_r2_r1 = new ModelRenderer(this);
		gun29_r2_r1.setRotationPoint(0.0F, -44.2766F, -41.3398F);
		gun.addChild(gun29_r2_r1);
		setRotationAngle(gun29_r2_r1, -2.3562F, 0.0F, 0.0F);
		gun29_r2_r1.cubeList.add(new ModelBox(gun29_r2_r1, 64, 6, -3.0F, -30.7F, -11.5F, 3, 2, 2, 0.0F, false));

		gun29_r1_r1 = new ModelRenderer(this);
		gun29_r1_r1.setRotationPoint(0.0F, -40.2059F, -31.8537F);
		gun.addChild(gun29_r1_r1);
		setRotationAngle(gun29_r1_r1, -2.3562F, 0.0F, 0.0F);
		gun29_r1_r1.cubeList.add(new ModelBox(gun29_r1_r1, 32, 52, -3.0F, -27.7F, -9.6F, 3, 1, 2, 0.0F, false));

		gun30_r1_r1 = new ModelRenderer(this);
		gun30_r1_r1.setRotationPoint(0.0F, -47.0343F, -34.6821F);
		gun.addChild(gun30_r1_r1);
		setRotationAngle(gun30_r1_r1, -2.3562F, 0.0F, 0.0F);
		gun30_r1_r1.cubeList.add(new ModelBox(gun30_r1_r1, 75, 52, -3.0F, -32.7F, -8.6F, 3, 2, 1, 0.0F, false));

		gun21_r1_r1 = new ModelRenderer(this);
		gun21_r1_r1.setRotationPoint(0.0F, -9.4763F, -29.2822F);
		gun.addChild(gun21_r1_r1);
		setRotationAngle(gun21_r1_r1, -1.1781F, 0.0F, 0.0F);
		gun21_r1_r1.cubeList.add(new ModelBox(gun21_r1_r1, 0, 31, -2.5F, -26.65F, -7.55F, 2, 2, 1, 0.0F, false));

		gun22_r1_r1 = new ModelRenderer(this);
		gun22_r1_r1.setRotationPoint(0.0F, -27.2148F, 17.4296F);
		gun.addChild(gun22_r1_r1);
		setRotationAngle(gun22_r1_r1, 1.1781F, 0.0F, 0.0F);
		gun22_r1_r1.cubeList.add(new ModelBox(gun22_r1_r1, 30, 13, -2.5F, -26.65F, -12.65F, 2, 2, 1, 0.0F, false));
	}

	@Override
	public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5) {
		receiver.render(f5);
		gun.render(f5);
	}

	public void setRotationAngle(ModelRenderer modelRenderer, float x, float y, float z) {
		modelRenderer.rotateAngleX = x;
		modelRenderer.rotateAngleY = y;
		modelRenderer.rotateAngleZ = z;
	}
}