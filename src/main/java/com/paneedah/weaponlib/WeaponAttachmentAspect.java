...
import com.paneedah.weaponlib.state.*;
...
public final class WeaponAttachmentAspect implements Aspect<WeaponState, PlayerWeaponInstance> {
	...
	public static class ChangeAttachmentPermit extends Permit<WeaponState> {
		...
		public ChangeAttachmentPermit(AttachmentCategory attachmentCategory, ItemStack attachment) {
	...
	public void setStateManager(StateManager<WeaponState, ? super PlayerWeaponInstance> stateManager) {
		...
		this.stateManager = stateManager
				...
				.withAction((instance) -> permitManager.request(new ExitAttachmentModePermit(WeaponState.READY), instance, (p, e) -> {/* do nothing on callback */})).manual()
	...
	public void changeAttachment(AttachmentCategory attachmentCategory, PlayerWeaponInstance weaponInstance) {
		...
		stateManager.changeState(this, weaponInstance, new ChangeAttachmentPermit(attachmentCategory), WeaponState.NEXT_ATTACHMENT);
	}
	...
	public void forceAttachment(AttachmentCategory attachmentCategory, PlayerWeaponInstance weaponInstance, ItemStack attachment) {
		...
		stateManager.changeState(this, weaponInstance, new ChangeAttachmentPermit(attachmentCategory, attachment), WeaponState.NEXT_ATTACHMENT);
	}
	...
	private void changeAttachment(ChangeAttachmentPermit permit, PlayerWeaponInstance weaponInstance) {
		...
		final EntityPlayer player = (EntityPlayer) weaponInstance.getPlayer();
		...
		if (lookupResult.index >= 0) {
			...
			if (player.inventory.getStackInSlot(lookupResult.index) == ItemStack.EMPTY)
				return;
		...
		if (currentAttachment != null && !player.isCreative()) {
			...
			} else if (player.inventory.mainInventory.get(lookupResult.index) == ItemStack.EMPTY) {
			...
			if (ItemStack.areItemStacksEqual(inventory, stack)) {
				...
				*/
				...
				if (compatibleAttachment == null
						...
						|| !hasRequiredAttachments(compatibleAttachment.getAttachment(), weaponInstance)
						|| inventory.getItem() == currentAttachment) {
					continue;
				}
	...
	private AttachmentLookupResult next(AttachmentCategory category, Item currentAttachment, PlayerWeaponInstance weaponInstance) {
		...
		final byte[] originallySelectedAttachmentIndexes = weaponInstance.getSelectedAttachmentIndexes();
		...
		final boolean isCategoryRemovable = weaponInstance.getWeapon().isCategoryRemovable(category);
		result.index = -1;
		final int offset = activeIndex + 1;

		final int endIndex = isCategoryRemovable ? (INVENTORY_SIZE + 1) : INVENTORY_SIZE;
		...
		for (int i = 0; i < endIndex; i++) {
			...
			final ItemStack slotItemStack = ((EntityPlayer) weaponInstance.getPlayer()).inventory.getStackInSlot(currentIndex);
	...
	public static boolean hasRequiredAttachments(ItemAttachment<Weapon> attachmentItemFromInventory, PlayerWeaponInstance weaponInstance) {
		final List<ItemAttachment<Weapon>> requiredAttachments = attachmentItemFromInventory.getRequiredAttachments();
		...
		for (int currentAttachmentId : weaponInstance.getActiveAttachmentIds()) {
			final Item attachmentItem = Item.getItemById(currentAttachmentId);
	...
	public static boolean isRequired(ItemAttachment<Weapon> attachmentItem, PlayerWeaponInstance weaponInstance) {
		...
		for (int currentAttachmentId : weaponInstance.getActiveAttachmentIds()) {
			final Item otherAttachmentItem = Item.getItemById(currentAttachmentId);
	...
	public static ArrayList<ItemAttachment<Weapon>> whatRequiredFor(ItemAttachment<Weapon> attachmentItem, PlayerWeaponInstance weaponInstance) {
		final ArrayList<ItemAttachment<Weapon>> requireesList = new ArrayList<>();
	...
	public static void addAttachment(ItemAttachment<Weapon> attachment, PlayerWeaponInstance weaponInstance) {
		...
		if (currentAttachment == null) {
			...
			LOGGER.error("Attachment of category {} installed, remove it first", attachment.getCategory());
		}
	}
