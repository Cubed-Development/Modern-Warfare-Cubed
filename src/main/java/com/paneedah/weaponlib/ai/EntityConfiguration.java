        ...
        public void register(ModContext context) {
            ...
            if (ModernConfigManager.terroristEquipmentConfiguration == null || ModernConfigManager.terroristEquipmentConfiguration.isEmpty()) {
                ...
                for (String attachment : attachments) {
                    ...
                    if (parts.length < 2) {
                        LOGGER.warn("Invalid attachment configuration for entity {}: {}. Expected format: <gunId>:<weight>[:<attachment>...]", name, attachment);
                        continue;
                    }
                    ...
                    try {
                        ...
                        LOGGER.warn("Invalid weight for gun {}: {}. Expected a valid double.", name, parts[1]);
                        continue;
                    }

                    Item gun = Item.REGISTRY.getObject(new ResourceLocation(ID, gunId));
                    if (gun == null) {
                        LOGGER.warn("Invalid equipment for entity {}: {}. Expected a valid item.", name, gunId);
                        continue;
                    }
                    ...
                    if (parts.length >= 3) {
                        for (String attachmentId : Arrays.asList(parts).subList(2, parts.length)) {
                            ...
                            if (!(att instanceof ItemAttachment)) {
                                LOGGER.warn("Invalid attachment for entity {}: {}. Expected a valid item.", name, attachmentId);
                                continue;
                            }
