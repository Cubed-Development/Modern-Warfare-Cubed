 ...
 * Make sure to enable
 * <p>
 * <a href="https://stackoverflow.com/questions/31129331/how-can-i-get-the-parameter-name-in-java-at-run-time">...</a>
 *
...
public class CompatibilityClassGenerator {
    ...
    public StringBuilder getIfStatementChecks(String name, String caseName) {
        String[] args = PATTERN.split(name);
    ...
    public StringBuilder buildOutMethod(Pair<Class<?>, Method> original, String searchTerm) {
        ...
        builder.append("public static ").append(original.getSecond().getReturnType().getSimpleName()).append(" ").append(original.getSecond().getName());
        builder.append("(");
        ...
        for (int i = 0; i < parameters.length; ++i) {
            ...
            if (i == 0) {
                builder.append(parameters[i].getType().getSimpleName()).append(" ").append(parameters[i].getName());
                parameterBuilder.append(parameters[i].getName());

            } else {
                builder.append(", ").append(parameters[i].getType().getSimpleName()).append(" ").append(parameters[i].getName());
                parameterBuilder.append(", ").append(parameters[i].getName());
            }
        }
        ...
        for (Pair<Class<?>, Method> pair : methods) {
            ...
            if (!original.getSecond().getReturnType().toString().equals("void")) {
                builder.append("\n\t\t\treturn ").append(genericName).append(".").append(pair.getSecond().getName()).append(parameterBuilder).append(";");
            } else {
                builder.append("\n\t\t\t").append(genericName).append(".").append(pair.getSecond().getName()).append(parameterBuilder).append(";");
                builder.append("\n\t\t\tbreak;");
            }
