...
public class CustomGui extends CompatibleGui {
	...
	public void handleVehicleHUD(RenderGameOverlayEvent.Pre event) {
		...
		if(mc.player.isRiding() && mc.player.getRidingEntity() instanceof EntityVehicle) {
			...
			if(!Double.isNaN(v.getSolver().getVelocityVector().lengthSquared()) && v.getSolver().getVelocityVector().lengthSquared() != 0.0 && (eventType == CROSSHAIRS || eventType == HOTBAR || eventType == HEALTH || eventType == EXPERIENCE || eventType == ARMOR))
				event.setCanceled(true);
		}
	...
	public void handleHelmetHUD(RenderGameOverlayEvent.Pre event) {
		...
		if (chestStack != null && helmetStack != null && feetStack != null && chestStack.getItem() instanceof CustomArmor && helmetStack.getItem() instanceof CustomArmor && feetStack.getItem() instanceof CustomArmor && ((CustomArmor)chestStack.getItem()).getUnlocalizedArmorSetName().equals(((CustomArmor) helmetStack.getItem()).getUnlocalizedArmorSetName()) && ((CustomArmor)chestStack.getItem()).getUnlocalizedArmorSetName().equals(((CustomArmor) feetStack.getItem()).getUnlocalizedArmorSetName())) {
			...
			if(maxShieldCapacity > 0) {
				...
				drawShieldIndicator(armor, CompatibleMathHelper.clamp_double(currentShieldCapacity / maxShieldCapacity, 0.0, 1.0), screenWidth, screenHeight);
			}
		}
	...
	public void handleAmmoCounter(RenderGameOverlayEvent.Pre event, PlayerWeaponInstance weaponInstance, double scaledWidth, double scaledHeight) {		final int AMMO_COUNTER_Y_POS = 128;
		...
		int totalCapacity;
		if (magazine != null) totalCapacity = magazine.getAmmo();
		else totalCapacity = weaponInstance.getWeapon().getAmmoCapacity();
	...
	public void handleOpenDoorHUD(RenderGameOverlayEvent.Pre event, double scaledWidth, double scaledHeight) {
		...
		if(VectorTools.vectorsEqual(cachedPlayerPosition, player.getPositionVector()) && cachedPlayerEyeHeight == player.getEyeHeight() && VectorTools.vectorsEqual(cachedLookVector, player.getLookVec()))
			return;
	...
	public void onCompatibleRenderCrosshair(RenderGameOverlayEvent.Pre event) {
		...
		else if(itemStack.getItem() instanceof ItemWirelessCamera || itemStack.getItem() instanceof ItemGrenade) {
            mc.entityRenderer.setupOverlayRendering();
            int color = 0xFFFFFF;
    ...
    private void drawShieldIndicator(CustomArmor armor, double capacity, double screenWidth, double screenHeight) {
        ...
        drawTexturedQuadFit((armor.getShieldIndicatorPositionX() - 1) * (screenWidth / 640.0), armor.getShieldIndicatorPositionY() * (screenHeight / 328.0), armor.getShieldIndicatorWidth() * (screenWidth / 640.0) + 2, (armor.getShieldIndicatorHeight() + 1) * (screenHeight / 328.0), -101);
        ...
        drawTexturedQuadFit((armor.getShieldIndicatorPositionX() - 1) * (screenWidth / 640.0), armor.getShieldIndicatorPositionY() * (screenHeight / 328.0), ((armor.getShieldIndicatorWidth() + 2) * (screenWidth / 640.0)) * capacity, (armor.getShieldIndicatorHeight() + 1) * (screenHeight / 328.0), -101);
	...
	private String getDefaultMagazineMessage(ItemStack itemStack) {
		...
		return compatibility.getLocalizedString("gui.ammoCounter", Tags.getAmmo(itemStack) + "/" + magazine.getAmmo());
	}
	...
	private String getDefaultWeaponMessage(PlayerWeaponInstance weaponInstance) {
		...
		String text;
		if(weaponInstance.getWeapon().getAmmoCapacity() == 0 && totalCapacity == 0) text = compatibility.getLocalizedString("gui.noMagazine");
		else text = compatibility.getLocalizedString("gui.ammoCounter", weaponInstance.getWeapon().getCurrentAmmo(compatibility.clientPlayer()) + "/" + totalCapacity);
