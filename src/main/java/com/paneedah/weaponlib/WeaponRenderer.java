...
//    private boolean magicMagPermit;
    ...
    public static class StateDescriptor implements MultipartRenderStateDescriptor<RenderableState, Part, RenderContext<RenderableState>> {
        ...
        protected final PlayerWeaponInstance instance;
        protected final MultipartRenderStateManager<RenderableState, Part, RenderContext<RenderableState>> stateManager;
    ...
    public static class Builder {
        ...
        public Builder withFirstPersonPositioning(Transform firstPersonTransform) {
            ...
            this.firstPersonPositioning = renderContext -> firstPersonTransform.doGLDirect();
            return this;
        }

        public Builder withFPSZooming(Transform zooming) {
            this.firstPersonZoomingTransform = zooming;
            this.firstPersonPositioningZooming = renderContext -> zooming.doGLDirect();
            return this;
        }
        ...
        public Builder withFirstPersonHandPositioning(Transform leftHand, Transform rightHand) {
            ...
            this.firstPersonLeftHandPositioning = renderContext -> leftHand.doGLDirect();
            this.firstPersonRightHandPositioning = renderContext -> rightHand.doGLDirect();
            return this;
        }

        public void checkDefaults() {
            if (firstPersonTransform == null) {
                firstPersonTransform = Transform.ZERO.copy();
            }
            if (firstPersonLeftHandTransform == null) {
                firstPersonLeftHandTransform = Transform.ZERO.copy();
            }
            if (firstPersonRightHandTransform == null) {
                firstPersonRightHandTransform = Transform.ZERO.copy();
            }
        }
        ...
        public Builder setupModernMagazineAnimations(String animationFile, Part... parts) {
            ...
            for (Part p : parts) {
                ...
                Vector3F r = ((ItemMagazine) p).rotationPoint;
                ...
                if (hasUnloadEmpty) {
                    withUnloadEmptyCustom(p, BBLoader.getAnimation(animationFile, BBLoader.KEY_UNLOAD_EMPTY, BBLoader.KEY_MAGAZINE)
                            .getTransitionList(Transform.ZERO.copy().withRotationPoint(r.x, r.y, r.z), BBLoader.HANDDIVISOR));
                }
            ...
            if (hasCompoundReloadEmpty && compoundReloadEmptyUsesTactical) {
                withFPSCustomCompoundReloadingEmpty(SpecialAttachments.MagicMag.getRenderablePart(),
                        BBLoader.getAnimation(animationFile, BBLoader.KEY_COMPOUND_RELOAD_EMPTY, BBLoader.KEY_MAGIC_MAGAZINE)
                                .getTransitionList(Transform.ZERO
        ...
        public Builder setupBoltActionAnimations(ItemAttachment<Weapon> action, String animationFile, String partKey) {
            ...
            Vector3F rotPoint = action.rotationPoint;
            ...
            if (hasEjectSpentRound && set.getSingleAnimation(BBLoader.KEY_EJECT_SPENT_ROUND).hasBone(partKey)) {
                withFirstPersonCustomPositioningUnloading(aR15Action, BBLoader.getAnimation(animationFile, BBLoader.KEY_EJECT_SPENT_ROUND, partKey)
                        .getTransitionList(Transform.ZERO.copy().withRotationPoint(rotPoint.x, rotPoint.y, rotPoint.z), BBLoader.HANDDIVISOR));
            }
...
//                        DEFAULT_ANIMATION_DURATION);
    ...
    public void renderItem(ItemStack weaponItemStack, RenderContext<RenderableState> renderContext, Positioner<Part, RenderContext<RenderableState>> positioner) {
        ...
        boolean shot = false;
        if (renderContext.getEntityLiving() != null && (ClientEventHandler.checkShot(renderContext.getEntityLiving().getEntityId()) || AnimationGUI.getInstance().forceFlash.isState())) {
            shot = true;
            ...
            MuzzleFlashRenderer.renderFlash(renderContext.getEntityLiving().getEntityId(), weaponItemStack, false);
        ...
        if (getBuilder().getModel() instanceof ModelWithAttachments) {
            attachments = ((Weapon) weaponItemStack.getItem()).getActiveAttachments(renderContext.getEntityLiving(), weaponItemStack);
        }
        ...
        if (getBuilder().getTextureName() != null)
            ...
            MC.renderEngine.bindTexture(new ResourceLocation(ID + ":textures/models/" + ((Weapon) weaponItemStack.getItem()).getTextureName()));
        ...
        if (!OpenGLSelectionHelper.isInSelectionPass && !AnimationGUI.getInstance().magEdit.isState()) {
            ...
            if (useSkin) {
                ...
                if (itemSkin.getTexturedModels().get(0).getTextureName().startsWith("customskin_")) {
                    MC.getTextureManager().bindTexture(CustomSkin.getCustomSkinResource(itemSkin.getTexturedModels().get(0).getTextureName().replace("customskin_", "")));
                } else {
                    MC.getTextureManager().bindTexture(new ResourceLocation(ID + ":textures/models/" + itemSkin.getTexturedModels().get(0).getTextureName()));
                }
        ...
        if (owner != null && owner != MC.player) {
            ...
            sqDistance = projectView.squareDistanceTo(owner.posX, owner.posY, owner.posZ);
        }
        ...
        if (!AnimationModeProcessor.getInstance().shouldIsolateCategory()) {
            ...
            if (getBuilder().getModel() != null) {
                getBuilder().getModel().render(this.owner,
    ...
    public void renderAttachments(Positioner<Part, RenderContext<RenderableState>> positioner, RenderContext<RenderableState> renderContext, List<CompatibleAttachment<? extends AttachmentContainer>> attachments) {
        ...
        // Todo: Wow this is bad we iterate two times instead of one? why the hell?
    ...
    // Todo: sus
    public void setMagicMagPermit(final boolean state) {
//        magicMagPermit = state;
    ...
    private void renderCompatibleAttachment(final CompatibleAttachment<?> compatibleAttachment, final Positioner<Part, RenderContext<RenderableState>> positioner, final RenderContext<RenderableState> renderContext) {
        ...
        if (compatibleAttachment.getAttachment().getCategory() == AttachmentCategory.MAGICMAG) {
            final WeaponState state = renderContext.getWeaponInstance().getState();
            ...
            boolean time = System.currentTimeMillis() - this.magicAnimationTimer >= renderContext.getWeaponInstance().getAnimationDuration(magicState) * 1.2;
...
//                return;
            ...
            if (currentMagazine != null) {
                //compatibleAttachment.getModelPositioning().accept(texturedModel.getA());

                //currentMagazine.getModelPositioning().accept(arg0);
            }
        }

        if (AnimationGUI.getInstance().magEdit.isState() && !OpenGLSelectionHelper.isInSelectionPass && compatibleAttachment.getAttachment() instanceof ItemMagazine) {
            GlStateManager.pushMatrix();
        ...
        else if (compatibleAttachment.getPositioning() instanceof Runnable)
            ((Runnable) compatibleAttachment.getPositioning()).run();
        ...
        renderTexturedModel(compatibleAttachment, renderContext, compatibleAttachment.getAttachment().getTexturedModels());
        renderTexturedModel(compatibleAttachment, renderContext, compatibleAttachment.getAttachment().getOnWeaponTexturedModels());

        for (final CustomRenderer<?> preloaded : compatibleAttachment.getAttachment().getAllPostRenderers())
            ((CustomRenderer<RenderableState>) preloaded).render(renderContext);
    ...
    private void renderTexturedModel(final CompatibleAttachment<?> compatibleAttachment, final RenderContext<RenderableState> renderContext, List<TexturedModel> texturedModels) {
        for (final TexturedModel texturedModel : texturedModels) {
            GlStateManager.pushMatrix();
            ...
            if (compatibleAttachment.getModelPositioning() != null) {
                if (compatibleAttachment.getAttachment().getCategory() == AttachmentCategory.MAGICMAG && currentMagazine != null)
                    currentMagazine.getModelPositioning().accept(texturedModel.getModel());
                else
                    compatibleAttachment.getModelPositioning().accept(texturedModel.getModel());

                final Builder rendererBuilder = renderContext.getWeaponInstance().getWeapon().getRenderer().getBuilder();
                final ItemAttachment<Weapon>[] possibleActionList = rendererBuilder.actionPiece;

                if (possibleActionList != null) {
                    final Optional<ItemAttachment<Weapon>> optionalAttachment = Arrays.stream(possibleActionList).filter(part -> compatibleAttachment.getAttachment() == part).findFirst();

                    if (optionalAttachment.isPresent()) {
                        final boolean debug = DebugCommand.isDebuggingActionPosition();

                        final Transform transform = debug ? DebugCommand.debugSlideTransform : rendererBuilder.actionPieceTransform;
    ...
    public static void applyRotationAtPoint(float xOffset, float yOffset, float zOffset, float xRotation,
        ...
        GL11.glRotatef(xRotation, 1f, 0, 0);
        GL11.glRotatef(yRotation, 0, 1f, 0);
        GL11.glRotatef(zRotation, 0, 0, 1f);
    ...
    public void renderItem() {
        ...
        final RenderContext<RenderableState> renderContext = new RenderContext<>(owner, itemStack);
        ...
        switch (transformType) {
			...
			*/
                ...
                StateDescriptor thirdPersonStateDescriptor1 = getThirdPersonStateDescriptor(owner, itemStack);

                renderContext.setPlayerItemInstance(thirdPersonStateDescriptor1.instance);

                MultipartPositioning<Part, RenderContext<RenderableState>> multipartPositioning2 = thirdPersonStateDescriptor1.stateManager.nextPositioning();

                renderContext.setTransitionProgress(multipartPositioning2.getProgress());
                renderContext.setFromState(multipartPositioning2.getFromState(RenderableState.class));
                renderContext.setToState(multipartPositioning2.getToState(RenderableState.class));

                positioner = multipartPositioning2.getPositioner();
            ...
            case THIRD_PERSON_LEFT_HAND:
                ...
                if (owner instanceof EntityPlayer && !MWCUtil.isProning((EntityPlayer) owner)) {
                    StateDescriptor thirdPersonStateDescriptor = getThirdPersonStateDescriptor(owner, itemStack);
            ...
            case FIRST_PERSON_LEFT_HAND:
                ...
                StateDescriptor stateDescriptor = getFirstPersonStateDescriptor(owner, itemStack);
                ...
                if (!OpenGLSelectionHelper.isInSelectionPass && AnimationModeProcessor.getInstance().getFPSMode()) {
                    GlStateManager.pushMatrix();
                    ResourceLocation resourceLocation = new ResourceLocation(ID + ":textures/hud/grid.png");
			...
			*/
			...
			if (!AnimationModeProcessor.getInstance().getFPSMode()) {
                ...
                // + "shaders/gunlight"));
                if (owner != null && owner.getHeldItemMainhand() != null && owner.getHeldItemMainhand().getItem() instanceof Weapon) {
                    // Draw hands only if weapons is held in the main hand
                    /*
                     */
                    if (OpenGLSelectionHelper.isInSelectionPass) {
                        if (OpenGLSelectionHelper.shouldRender(1)) {
                            ...
                            renderLeftArm(owner, renderContext, positioner);

                        }
                        if (OpenGLSelectionHelper.shouldRender(2)) {
                            OpenGLSelectionHelper.bindSelectShader(2);
                            renderRightArm(owner, renderContext, positioner);
    ...
    public static void fixVersionSpecificFirstPersonPositioning(ItemCameraTransforms.TransformType transformType) {
        ...
        GL11.glTranslatef(0f, 0.6f, 0f); // -0.6 y-offset is copy somewhere upstream in 1.9.4, so adjusting it
...
//        GlStateManager.translate(0.0F, 0.0F, -2000.0F);
    ...
    public static <T> void renderRightArm(EntityLivingBase player, RenderContext<T> renderContext, Positioner<Part, RenderContext<T>> positioner) {
        Render<AbstractClientPlayer> entityRenderObject = MC.getRenderManager().getEntityRenderObject((AbstractClientPlayer) player);
    ...
    // Todo: Arms the weapon of all time
    public static <T> void renderLeftArm(EntityLivingBase player, RenderContext<T> renderContext, Positioner<Part, RenderContext<T>> positioner) {
