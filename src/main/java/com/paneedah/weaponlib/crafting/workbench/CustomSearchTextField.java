...
public class CustomSearchTextField extends GuiTextField {
    ...
    public void drawTextBox() {
        ...
        if (getText().isEmpty() && !isFocused()) {
            drawString(MC.fontRenderer, hintText, x + 3, y + 2, 0xFFFFFF);

        } else if (isFocused() && getText().isEmpty()) {
            if (System.currentTimeMillis() % 1000 < 500) {
                drawString(MC.fontRenderer, "_", x + 3, y + 2, 0xFFFFFF);
            }

        } else if (!getText().isEmpty()) {
            drawString(MC.fontRenderer, getText(), x + 3, y + 2, 0xFFFFFF);
            if (System.currentTimeMillis() % 1000 < 500) {
                drawString(MC.fontRenderer, "_", x + 3 + MC.fontRenderer.getStringWidth(getText()), y + 2, 0xFFFFFF);
            }
        }
    }
}
