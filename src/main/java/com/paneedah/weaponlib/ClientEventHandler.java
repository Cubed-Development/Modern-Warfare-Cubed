...
public class ClientEventHandler extends CompatibleClientEventHandler {
	...
	public ClientEventHandler(ClientModContext modContext, Lock mainLoopLock, SafeGlobals safeGlobals, Queue<Runnable> runInClientThreadQueue /*, ReloadAspect reloadAspect*/) {
    ...
    private void update() {
		...
		if(isProning(player)) slowPlayerDown(player, SLOW_DOWN_WHILE_PRONING_ATTRIBUTE_MODIFIER);
		else restorePlayerSpeed(player, SLOW_DOWN_WHILE_PRONING_ATTRIBUTE_MODIFIER);
		
		if(mainHandHeldWeaponInstance != null) {
			if(player.isSprinting())
				mainHandHeldWeaponInstance.setAimed(false);

			if(mainHandHeldWeaponInstance.isAimed()) slowPlayerDown(player, SLOW_DOWN_WHILE_ZOOMING_ATTRIBUTE_MODIFIER);
			else restorePlayerSpeed(player, SLOW_DOWN_WHILE_ZOOMING_ATTRIBUTE_MODIFIER);
		...
		if(spreadableExposure != null && spreadableExposure.getTotalDose() > SLOW_DOWN_WHEN_POISONED_DOSE_THRESHOLD) slowPlayerDown(player, SLOW_DOWN_WHILE_POISONED_ATTRIBUTE_MODIFIER);
		else restorePlayerSpeed(player, SLOW_DOWN_WHILE_POISONED_ATTRIBUTE_MODIFIER);
	...
	private void restorePlayerSpeed(EntityPlayer entityPlayer, AttributeModifier modifier) {
		if(entityPlayer.getEntityAttribute(compatibility.getMovementSpeedAttribute()).getModifier(modifier.getID()) != null)
			entityPlayer.getEntityAttribute(compatibility.getMovementSpeedAttribute()).removeModifier(modifier);
	}

	// TODO: create player utils, move this method
	private void slowPlayerDown(EntityPlayer entityPlayer, AttributeModifier modifier) {
		if(entityPlayer.getEntityAttribute(compatibility.getMovementSpeedAttribute()).getModifier(modifier.getID()) == null)
			entityPlayer.getEntityAttribute(compatibility.getMovementSpeedAttribute()).applyModifier(modifier);
	}
	...
	public void onCompatibleRenderHand(CompatibleRenderHandEvent event) {
	    if(mc.gameSettings.thirdPersonView == 0 && !compatibility.isShadersModEnabled()) {
	        PlayerWeaponInstance weaponInstance = modContext.getMainHeldWeapon();
	        DynamicShaderContext shaderContext = new DynamicShaderContext(DynamicShaderPhase.PRE_ITEM_RENDER, null, mc.getFramebuffer(), event.getPartialTicks()).withProperty("weaponInstance", weaponInstance);
    ...
    protected void onCompatibleRenderTickEvent(CompatibleRenderTickEvent event) {
        DynamicShaderContext shaderContext = new DynamicShaderContext(DynamicShaderPhase.POST_WORLD_RENDER, mc.entityRenderer, mc.getFramebuffer(), event.getRenderTickTime());
        EntityPlayer clientPlayer = compatibility.clientPlayer();

		if(event.getPhase() == CompatibleRenderTickEvent.Phase.END) {
        ...
        if(event.getPhase() == CompatibleRenderTickEvent.Phase.START ) {
			...
			PlayerItemInstance<?> instance = modContext.getPlayerItemInstanceRegistry().getMainHandItemInstance(clientPlayer);
