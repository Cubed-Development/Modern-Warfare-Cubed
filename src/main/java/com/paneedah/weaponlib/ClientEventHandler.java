...
public class ClientEventHandler {
    ...
    public final void onRenderTickEvent(TickEvent.RenderTickEvent renderTickEvent) {
        final DynamicShaderContext dynamicShaderContext = new DynamicShaderContext(DynamicShaderPhase.POST_WORLD_RENDER, MC.entityRenderer, MC.getFramebuffer(), renderTickEvent.renderTickTime);
        final EntityPlayer clientPlayer = MC.player;

        if (renderTickEvent.phase == TickEvent.RenderTickEvent.Phase.START) {
            ClientModContext.currentContext = modContext;

            if (clientPlayer != null) {
                final PlayerItemInstance<?> playerItemInstance = modContext.getPlayerItemInstanceRegistry().getMainHandItemInstance(clientPlayer);

                final DynamicShaderGroupSource dynamicShaderGroupSource = pipelineShaderGroupSourceProvider.getShaderSource(dynamicShaderContext.getPhase());

                if (dynamicShaderGroupSource != null)
                    shaderGroupManager.loadFromSource(dynamicShaderContext, dynamicShaderGroupSource);

                if (playerItemInstance != null) {
                    final Perspective perspective = PerspectiveManager.getPerspective(playerItemInstance, true);
        ...
        } else if (renderTickEvent.phase == TickEvent.RenderTickEvent.Phase.END) {
            ClientProxy.renderingPhase = null;
            shaderGroupManager.removeStaleShaders(dynamicShaderContext);
        }
    }
